...
class _WebsocketFilter(logging.Filter):
    ...
    def __init__(self):
        ...
        self.ignore_by_ends = [".server_logger"]
...
def handle_streams():
    ...
    stream_formatter = logging.Formatter(" - ".join(["%(asctime)s", "%(threadName)s", "%(name)s",
                                                     "[%(filename)s:%(lineno)d]", "%(levelname)s", "%(message)s"]))
    stream_handler.setFormatter(stream_formatter)
    logging.root.addHandler(stream_handler)
    logging.debug(
        "Added logging.StreamHandler object: {}".format(stream_handler))
...
def handle_sockets(websocket_connections, allow_broadcasts, socket_filters):
    ...
    logging.info(
        "Adding logging handler for websockets {} broadcasts.".format(preposition))
    websocket_handler = _WebsocketHandler(
        websocket_connections, allow_broadcasts)
    ...
    if socket_filters is not None:
        ...
        for filter in socket_filters:
            logging.debug("Adding filter item number: {}".format(
                socket_filters.index(filter)))
            if isinstance(filter, logging.Filter):
                websocket_handler.addFilter(filter)
            else:
                msg = "Can't add logging filter with type: {}".format(
                    type(filter))
                logging.error(msg)
                raise TypeError(msg)
